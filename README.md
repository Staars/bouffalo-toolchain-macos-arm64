...
- The workflow patches binutils' bundled zlib (`fdopen` define poisons modern macOS SDK headers) and GDB's `enum-flags.h` (new Clang rejects the `T(-1)` signedness probe) — see the `Patch for macOS ARM64` step.
